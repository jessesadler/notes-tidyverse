...
# RStudio conf 2022 ggplot2 workshop
...
## Label basics {#sec-label-basics}

For more detailed info on labels, see @sec-labels.
...
- Positions: `scale_x_*()` and `scale_y_*()`
  - `continuous()`, `discrete()`, `reverse()`, `log10()`, `sqrt()`, `date()`
- Colors: `scale_color_*()` and `scale_fill_*()`
  - `continuous()`, `discrete()`, `manual()`, `gradient()`, `gradient2()`, `brewer()`
- Sizes: `scale_size_*()` and `scale_radius_*()`
  - `continuous()`,` discrete()`, `manual()`, `ordinal()`, `area()`, `date()`
- Shapes: `scale_shape_*()` and `scale_linetype_*()`
  - `continuous()`, `discrete()`, `manual()`, `ordinal()`
- Transparency: `scale_alpha_*()`
  - `continuous()`,` discrete()`, `manual()`, `ordinal()`, `date()`
...
### Arguments {#sec-scale-arguments}

- `name`: name used for the axis or legend title. If `NULL` the name will be omitted.
- `breaks`: numeric vector or function that takes the limits if the input and returns breaks.
...
- `limits`: numeric vector of length two providing minimum and maximum with `NA` to refer to the existing minimum or maximum. Or a function that accepts the existing (automatic) limits and returns new limits.
  - Setting limits with scales removes data outside of the limits. If you want to zoom, use the `limit` argument in the coordinate system, see @sec-coord-limit.
...
### Plot limits {#sec-coord-limit}
You can set plot limits with the coordinate functions. This zooms into the plot instead of removing data that falls outside of those limits as is done with the `scale_*()` functions, see @sec-scale-arguments.
...
### Flipped coordinate system {#sec-coord-flip}
...
### Circular coordinate system {#sec-coord-polar}
...
### Example: lillipop plot {#sec-lollipop}
...
### Transform a coordinate system {#sec-coord-trans}
...
## Labels in plot elements {#sec-labels}
See @sec-label-basics for basic discussion of labels.

Create labels with `labs()` as shown in @sec-label-basics
...
### Labels and theme {#sec-labels-theme}
...
### Labels and scales {#sec-labels-scales}
...
### Labels and markdown {#sec-labels-ggtext}
...
### Labels and facets {#sec-labels-facets}
```{r}
#| label: labels-facet
codes <- c(
  `TRUE` = "Workday",
...
### Annotation basics {#sec-anootation-basics}
Adding text annotations with `annotate()` and selecting the `geom` used for the annotation.
...
### Highlighting aspects with annotations {#sec-ann-highlights}
...
### Annotations with geoms {#sec-annotation-geoms}
...
### Text annotations {#sec-text-annotations}
...
### Annotations with `ggforce` {#sec-ggforce}
...
### Annotations example: Palmer penguins {#sec-annotations-ex}
...
Create multi-colored sequential palettes
...
### Build your own palettes: discrete palette
...
### Build your own palettes: continuous palette
...
## Legend placement and styling {#sec-legend}

**Guides** are the combination of scales and legends. Legend position is set within `theme()`, while many aspects of the styling of the legend can be set within `guide` argument of `scale_*()` functions. See `guides()` function for details.

Removing a legend from a plot:

- In `geom_*()`:`show.legend = FALSE`
- In `scale_*()`: `guide = "none"`
- In `guides()`: setting aesthetic to "none": `color = "none"`
- In `theme()` and `theme_()`: `legend.position = "none"`
...
### Legend and color example: color shading
