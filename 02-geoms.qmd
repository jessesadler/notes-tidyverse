...
The documentation for each geom provides the aesthetics that can be used with the geom. Aesthetics are not discussed in detail here. See @sec-geom-aes for an overview of aesthetic specifications from the `ggplot2` vignette.
...
Instead of using the default count stat, you can use `stat_identity()` which counts already provided in the data. `stat_identity()` requires both `x` and `y` positional aesthetics because it needs to know which variable is the count data.
...
`geom_col()` provides a shortcut for creating a bar plot using `stat_identity()`.
...
The `group` aesthetic determines which cases are connected together. Thus, when you want to draw multiple lines in a plot, you need to use `group` or another aesthetic such as `color` or `linetype` to create the groups. The difference between `geom_line()` and `geom_path()` can be shown by not including the `group` aesthetic. When you see a plot like the first one, you know that you are missing a `group` aesthetic.
...
Alongside `group` `color`, and `linetype` are the most important aesthetics for line plots. It is also worth noting that the width of the lines is altered with `linewidth` and not `size`.
...
`geom_ribbon()` is similar to `geom_line` but they create a polygon between a `ymin` and a `ymax`. `geom_area()` is a special case of `geom_ribbon()`, where the `ymin` is fixed to 0 and `y` is used instead of `ymax.`

A simple example is to subtract from and add to the `y` variable to create a buffer around `geom_line()`. You possibility would be to calculate quantiles to do this, though using `geom_smooth()` would be better. 
...
## Reference lines {#sec-geom-reference}
...
## Aesthetics {#sec-geom-aes}
See [Aesthetic specifications](https://ggplot2.tidyverse.org/articles/ggplot2-specs.html) for types of aesthetics and changes that can be made to those types.

### Color, fill, and alpha {#sec-geom-aes-color}
...
### Lines {#sec-geom-aes-lines}
The appearance of a line is affected by `linewidth`, `linetype`, `lineend`, `linejoin`.

`linewidth` is used to control the width or size of the line. 
...
### Points {#sec-geom-aes-points}
Points can be chosen in 5 ways:
...
### Grouping {#sec-geom-aes-groups}
The group aesthetic is by default set to the interaction of all discrete variables in the plot. This default is often enough, but when it does not work or when there is no discrete variable, you need to explicitly map `group` to a variable that has a different value for each group. See [Aesthetics: grouping](https://ggplot2.tidyverse.org/reference/aes_group_order.html)
