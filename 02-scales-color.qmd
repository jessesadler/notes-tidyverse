...
Most of the color packages have functions to list the name of the available palettes and to view the palettes as a plot. A particularly useful way to view of palette before using it is with `scales::show_col()`. Many of the packages also use the same structure as `scales` in creating/accessing palettes, and so it makes sense to show how this is done.
