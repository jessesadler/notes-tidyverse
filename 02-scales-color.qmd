...
Non-color scales are covered in [Chapter -@sec-scales]. Color is used in two aesthetics: `color` and `fill`. All of the color scales have a `scale_color_*()` function and an equivalent `scale_fill_*()` function.
...
The default color scales are provided to easy to use color scales. You can quickly choose the type of palette to use, but to manually set the colors of a scale it is better to use the functions for creating color scales.

- Continuous scales: `type` is "gradient" or "virdis"
  - `scale_color_continuous()`
  - `scale_fill_continuous()`
- Binned scales: `type` is "gradient" or "virdis"
  - `scale_color_binned()`
  - `scale_fill_binned()`
...
- Continuous scales: Gradient
  ...
  - `scale_color_gradientn()`: produces an n-color gradient.
...
`scale_color_gradientn()` is important for transforming a discrete palette into a continuous palette. This makes it the primary way to use color packages with continuous color scales.

#### Color brewer continuous scales
`scale_color_distiller()` provides a continuous palette using the Color Brewer palettes. Use the `type` argument to choose between sequential `"seq"`, diverging `"div"`, and qualitative `"qual"` palettes. This is a quick way to choose the default palette for each type and can be used with numerical manner for choosing a palette. Or just write the name of the palette in the `palette` argument.
...
You can access the virdis palette from `scale_color_continuous()`

```{r}
#| label: scale-continuous-virdis
pcont + 
...
`scale_color_virdis_b()` has the same ability to pick a palette `option`:
...
Use `scales::hue_pal()` to recreate or access the discrete color palette used for a plot, making sure to use the length of the discrete colors.
...
`scale_color_virdis_d()` provides the same set of viridis palettes in a discrete scale:
...
Most color palette packages have two main types of functions: **scale** functions that are usually named `scale_color_*()` and are meant to replace the `ggplot2` scale functions and **pal** functions that provide access to the hex values of palettes that can then be used with `scale_color_manual()`. The API for accessing these functions differs. This has led to the creation of `paletteer` (@sec-color-paletteer) to try to create a common interface for palettes.

Most of the color packages have functions to list the name of the available palettes and to view the palettes as a plot. A particularly useful way to view of palette before using it is with `scales::show_col()`.

### palettes pkg {#sec-solor-palettes-pkg}
A fun, quick alternative to `scales::show_col()` is to use the `palettes` package, which uses `vctrs` to print vectors of colors as hex rbg values with a color preview. `palettes` has its own way to access color palettes in `ggplot2`, but it is particularly useful for printing out a vector of colors with `pal_colour()`.
...
Once you have a palette, you can view the colors with `scales::show_col()`. This provides a good workflow to view the color palette and make any changes you want. Using `show_col()` is a good workflow for using any palette.
...
- `palette.colors(n, palette)`: Access to a set of qualitative palettes; default is "Okabe-Ito"
...
#### Continuous palettes
`hcl.colors()` is a simplified implementation of the color palettes from `colorspace`, see @sec-color-colorspace. This means that you can use the `colorspace` package to use these palettes directly in `ggplot2`.
...
The single hue are all inspired by Color Brewer except Oslo, which is from the `scico` palettes. 
...
- `hcl_palettes(plot = TRUE)`: View a plot of the palettes.cvd
...
The `scico` package provides access to the perceptually uniform and color-blindness friendly palettes developed by Fabio Crameri. The package has 35 different palettes. The color scales are sequential, and so it is a good augmentation to the viridis palettes.
...
`rcartocolor` provides color palettes made for mapping designed by [CARTO](https://carto.com/carto-colors/). Similar to Color brewer, the carto has sequential, diverging, and qualitative color palettes.
...
Use either the `MetBrewer` scale functions or the palettes within the `ggplot2` `scale_color_*()` functions.
...
`paletteer` provides a common API for accessing [dozens of palette packages](https://emilhvitfeldt.github.io/paletteer/index.html#included-packages) and thousands of color palette. The basic structure is `function("package::palette")`.
