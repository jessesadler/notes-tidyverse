...
- See Scales: Axes labels (@sec-scale-labels) for the discussion of guides in `scale_x/y_*()` functions and position labels.
- See Scales: Other scales (@sec-aesthetic-scales) for how to alter aesthetic scales, breaks, and labels used in legends.
- See Themes: Legends (@sec-theme-legend) for legend elements within `theme()`.
...
The easiest is probably with `labs()`, but the first argument for `scale_*()` and `guide_*()` functions are `name` for the scale name and `title` to be used for the legend title. The exception is `guide_colorsteps()`, which requires `title` to be a named argument argument.

### Merge and split legends {#sec-guide-split}
Legend titles are also the main way to merge or split legends. By default ggplot2 merges legends of the same variable, but they can be split by giving them different names.
...
One reason to change the type of legend is to switch between continuous, binned versions of legends. Note that this does not change the **scale** in the plot from continuous to binned but only its representation in the legend. See @sec-scale-size on changing the scale between continuous and binned within the plot itself.

For more details on using and tweaking the different types of legends see @sec-guide-styling.
...
## Legend label positioning {#sec-guide-labels}
...
#### Ticks
By default `guide_colorbar()` has tick marks to set off the breaks in the scale. `guide_colorsteps()` does not have tick marks by default since the breaks between the colors are more clear. Use the `ticks` argument to turn on or off tick marks and use `ticks.colour` and `ticks.linewidth` to control the ticks. Note the use of the British spelling of *colour* in the argument name.
...
See also Themes: Legends, @sec-theme-legend
