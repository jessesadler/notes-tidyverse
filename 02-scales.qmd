...
Scales control the details of how data values are mapped to aesthetic visual properties. Scales are applied to aesthetics (`aes()`) to determine how they look. They directly affect the size, color, position or shape of geoms. Scales are the basis for **guides** used to interpret the plot: axes and legends.

The scale functions all begin with `scale_`, then the name of the *aesthetic*, followed by the type of *scale*. E.g. `scale_x_continuous()` uses the continuous scale for the x-axis. This chapter is divided between positional scales (@sec-scale-positions) that affect the nature and labeling of the x and y axes and other aesthetic scales (@sec-aesthetic-scales) aside from color scales that are handled separately in @sec-color.
...
## Scale functions {#sec-scale-fns}

### Aesthetics {#sec-scale-aesthetics}
- Positions: `scale_x_*()` and `scale_y_*()` (@sec-scale-positions)
- Sizes: `scale_size_*()` and `scale_radius()` (@sec-scale-size)
- Shapes: `scale_shape_*()` (@sec-scale-shape)
- Linewidth: `scale_linewidth_*()` (@sec-scale-linewidth)
- Linetype: `scale_linetype_*()` (@sec-scale-linetype)
- Transparency: `scale_alpha_*()` (@sec-scale-alpha)
- Colors: `scale_color_*()` and `scale_fill_*()` (@sec-color)
...
What `guide` and `guide_axis()` add is the ability to change the `angle` of the labels or stack label. Changes to label angle can also be done with themes, see [Chapter -@sec-theme-axes].
...
`expansion()` takes vectors of either length 1 or 2 to either multiply (`mult`) or add (`add`) to the lower and upper limits of the scale. The below code adds ten units to the lower and upper bounds of the x scale and removes all padding from the lower bound of the y scale (multiplies by 0) and expands the upper bound of the y scale by 10%.
...
## Aesthetic scales {#sec-aesthetic-scales}
There are five main types of aesthetic scales (@sec-scale-types): continuous, binned, discrete, manual, and identity. The differences between them are largely self-explanatory. The exception is continuous and binned. Binning a continuous variable with `scale_*_binned()` changes the actual scale drawn on the plot and uses a binned legend style. This can be seen with `scale_size_binned()` below.
