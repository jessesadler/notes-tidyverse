...
One way to move the text/label away from the point is to manually nudge the them using `nudge_x` and `nudge_y`, which are scaled to the `x` and `y` axes.
...
Another way to alter the position of the text/label is to alter the alignment with `vjust` and `hjust`. See the [Aesthetic specifications vignette](https://ggplot2.tidyverse.org/articles/ggplot2-specs.html#justification) for more information. The arguments take:

- A number between 0 and 1:
  - `hjust`: left = 0, center = 0.5, right = 1
  - `vjust`: top = 1, middle = 0.5, bottom = 0
- Positional character:
  - `hjust`: `"left"`, `"center"`, or `"right"`
  - `vjust`: `"bottom"`, `"middle"`, or `"top"`
- Special position:
  - `"inward"`: towards the center
  - `"outward"`: away from the center

The default is center both `hjust` and `vjust`.

- **hjust**:
  - `"left"` justification text begins on the point and moves to the right.
  - `"right"` justification text ends on the point with the text to the left.
- **vjust**:
  - `"bottom"` justification the *bottom* of the text is on the point, moving the text *up*.
  - `"top"` justification the *top* of the text is on the point, moving the text *down*.
...
The special alignment parameters of `"inward"` and `"outward"` are particularly useful because they can move the text different ways depending upon where they are in the plot.
...
`ggrepel` uses an algorithmic approach to minimize overlap of points. This quickly and easily fixes the issues of labels overlapping the points so that you do not need to use nudge and alignment adjustments.
...
Line segments have many of the same adjustments as `geom_line()`. Arguments for lines are: 
...
You can make curved line segments and add arrows just like in `geom_curve()`. Arguments for curvature are:
...
One of the simplest and most effective forms of annotating a plot is to highlight a subset of the data that is particularly relevant for a certain point. There are a number of ways to do this.

### Color a subset of points {#sec-ann-highlight-color}

#### Gray out non-essential data
A simple example is to only add color to a subset of the points that you want to highlight, while graying out other data. For instance, we can highlight the Adelie and Chinstrap penguins that have flippers longer or equal to 205mm.
...
A similar affect is created by surrounding the points with an open circle shape.
...
#### Labels
`gghighlight` uses direct labeling by default, see @sec-ann-direct for more on this process. The label is chosen with the `label_key` argument. The label type is chosen with the `line_label_type` function with choices of `"ggrepel_label"`, `"ggrepel_text"`, `"text_path"`, `"label_path"`, or `"sec_axis"`.
...
Remove direct labels with `use_direct_label = FALSE`. You can add your own text labels to the filtered data because `gghighlight` replaces the plot's data with the filtered data.
...
See @sec-line-seg and @sec-geom-line-ref for more on lines and curves.
...
Left-hand curve with curve skewed towards beginning of line segment:
...
#### Arrows
Both `geom_segment()` and `geom_curve()` both have arguments for `arrow` to add arrows on one or both sides of the line segments using the `arrow()` function, which is exported from `grid::arrow()`. Arguments for controlling the aestheics of arrows:
...
The [ggtext](https://wilkelab.org/ggtext) package provides two geoms for creating text annotations with markdown and html rendering for ggplot2. In addition to the geoms, `ggtext` also provides a way to render markdown and html in plot theme elements, see @sec-theme-ggtext. In both the geoms and the theme elements `ggtext` provides two alternatives: markdown text labels and markdown text boxes with word wrapping. The actual rendering of the markdown and html is done by the [gridtext](https://wilkelab.org/gridtext/) package. It only supports a small subset of markdown/html capabilities. For a list of supported Markdown and HTML capabilities, see @sec-theme-html-support.
...
Use `fill = NA` and `label.color = NA` to remove the label box.
...
`geom_textbox()` is very similar to `geom_richtext()` but it automatically wraps text and does not support arbitrary rotation angles, though it can be rotated.
...
The `directlabels` package introduces a number of placement methods for allegorically placing labels on a plot using `geom_dl()`. See [here](https://tdhock.github.io/directlabels/docs/index.html) for a list of the possible placement methods. The most useful one for a scatter plot is `"smart.grid"`, which positions the labels near the center of each point cloud.
...
It is possible to use multiple justification parameters for the different lines with `scale_hjust_discrete()` and `scale_vjust_discrete()` and `scale_hjust_manual()` and `scale_vjust_manual()`. Use discrete options to automatically differ the horizontal or vertical justification of each group. Use manual options to pick the different levels of justification.
