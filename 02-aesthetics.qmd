...
**Aesthetics** define how variables in the data are mapped to visual properties through the `aes()` function. Everything inside `aes()` changes based on the data. Aesthetics can be specified for the entire plot (`ggplot(aes())`) or within each layer (`geom_point(aes())`). Within each layer you can add, override, or remove mappings.

## Mapping vs setting aesthetics {#sec-aes-map}
Aesthetics can either be mapped or set:

- **Map** an aesthetic to a variable with `aes()`.
- **Set** an aesthetic outside of `aes()` with a constant.

It is possible to map aesthetics to constants when you want to display multiple comparable layers. You can **name** the layers within `aes()` to distinguish them and create a legend.
